...
describe("fixture tutor and evaluation persistence", () => {
  it("preserves append-only tutor versions and isolated evaluation siblings after refresh", async () => {
    ...
    await evaluation.saveRun({ schemaVersion: "0.1", id: "run-1", projectId: "project-1", tutorVersionId: "tutor-1", scenarioIds: scenarios().map(({ id }) => id), status: "completed", readiness: "needs_revision", passCount: 1, warningCount: 0, completedAt: timestamp });
    await expect(evaluation.claimRunExecution({ projectId: "project-1", runId: "run-1" })).resolves.toMatchObject({ status: "running", readiness: "pending" });
