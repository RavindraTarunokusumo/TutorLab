...
import {
  getOrCreatePreviewConversation,
  PreviewConversationBusyError,
...
function error(error: unknown) {
  if (error instanceof PreviewConversationBusyError)
    return NextResponse.json(
      { error: "A preview update is already in progress" },
      { status: 409 },
    );
  ...
  if (error instanceof Error && /not found/i.test(error.message))
    return NextResponse.json(
      { error: "Tutor or conversation not found" },
      { status: 404 },
    );
  throw error;
}
...
export async function GET(
  ...
  try {
    ...
    const conversation = await getOrCreatePreviewConversation({
      projectId,
...
export async function POST(
  ...
  return withOpenAIRequestKey(request, async () => {
    try {
      ...
      const reply = await sendPreviewMessage({
        projectId: body.projectId,
        tutorVersionId: tutorId,
        conversationId: body.conversationId,
        message: body.message,
      });
      return new Response(streamPreviewReply(reply), {
        headers: {
...
export async function DELETE(
  ...
  try {
    ...
    await requireProjectAccess(request, body.projectId);
    return NextResponse.json({
      conversation: await resetPreviewConversation(body.projectId, tutorId),
    });
  } catch (caught) {
    return error(caught);
  }
