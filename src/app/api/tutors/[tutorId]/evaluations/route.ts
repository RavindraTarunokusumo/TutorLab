...
const RequestSchema = z.strictObject({
  ...
  scenarioIds: z.array(z.string().trim().min(1).max(96)).min(1).max(6).optional(),
});
...
function failure(error: unknown) {
  ...
  if (error instanceof Error && /not found|unavailable/i.test(error.message)) return NextResponse.json({ error: "Evaluation data is unavailable" }, { status: 404 });
  throw error;
}
...
export async function POST(request: Request, { params }: { params: Promise<{ tutorId: string }> }) {
  try {
    ...
    return NextResponse.json(await runTutorEvaluation({ ...input, tutorVersionId: tutorId, startOnly: true }), { status: 202 });
