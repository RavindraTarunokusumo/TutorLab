...
export async function POST(
  ...
  try {
    ...
    const existingRecommendations = evaluation.run.teacherRecommendations ?? [];
    ...
    if (!courseModels.findById) {
      return NextResponse.json({ error: "Course model is unavailable." }, { status: 404 });
    }
    ...
    if (error instanceof z.ZodError || error instanceof SyntaxError) {
      return NextResponse.json({ error: "Invalid recommendations request" }, { status: 400 });
    }
    throw error;
  }
}
