...
function clearRetainedRequestKey(projectId: string, tutorVersionId: string) {
  try {
    window.sessionStorage.removeItem(
      requestStorageKey(projectId, tutorVersionId),
    );
  } catch {
...
export function BuildProgress({
  ...
  const poll = useCallback(
    async (jobId: string, activeTutorVersionId: string) => {
      ...
      try {
        ...
        if (next.status === "running" || next.status === "pending") {
          pollTimer.current = window.setTimeout(
            () => void poll(jobId, activeTutorVersionId),
            1_000,
          );
          ...
          setStatus(
            "Scenario generation stopped safely. Retry reuses the same request.",
          );
        }
      } catch {
        if (!canUpdate(controller.signal)) return;
        setError(
          "Build progress could not be refreshed. Try again when the connection returns.",
        );
      }
    },
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!tutor) {
        ...
        setScenarios([]);
        setStatus(
          "Compile a tutor design before generating evaluation scenarios.",
        );
        return;
      }
      ...
      if (state.job?.status === "running" || state.job?.status === "pending") {
        ...
        pollTimer.current = window.setTimeout(
          () => void poll(state.job!.id, tutor.id),
          1_000,
        );
      } else if (
        state.scenarios.length === 6 ||
    ...
    try {
      ...
      setStatus(
        response.job.status === "completed"
          ? "Six evaluation scenarios are ready."
      ...
      if (
        ...
        pollTimer.current = window.setTimeout(
          () => void poll(response.job.id, tutorVersionId),
          1_000,
        );
      ...
      if (!canUpdate(controller.signal)) return;
      setError(
        "Evaluation scenarios could not be generated. You can safely try again.",
      );
      setStatus("");
    } finally {
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "The evaluation could not be completed. You can safely try again.",
      );
      setStatus("");
    } finally {
  ...
  const progress = Math.round(
    (job?.progress ?? (scenarioCount === 6 ? 1 : 0)) * 100,
  );
  ...
  if (!tutorVersionId && !error && status === "Loading durable build status…") {
    return (
      <section
        aria-busy="true"
        className="flex min-h-72 flex-col items-center justify-center rounded-xl border bg-card p-6 text-center"
      >
        <span aria-label="Loading build" className="processing-ring size-10" />
        <h1 className="mt-5 text-2xl font-semibold">Preparing tutor build</h1>
        <p className="mt-2 max-w-md text-muted-foreground">
          Loading the compiled tutor and build status.
        </p>
      </section>
    );
  }
  return (
    <section
      className="lg:grid lg:min-h-[calc(100dvh-7.5rem)] lg:grid-cols-[minmax(0,2fr)_minmax(20rem,1fr)]"
    ...
    >
      <div className="mx-auto w-full max-w-4xl space-y-5 px-4 py-10 sm:px-8">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            Track the persisted tutor build stages and prepare the six
        ...
        <ol className="space-y-3" aria-label="Durable build stages">
          <li className="rounded-xl border bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="font-medium">Tutor design</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {tutorVersionId
                    ? "A design was selected for the active tutor version."
                    : "Select a design after the course model is ready."}
                </p>
              </div>
              {tutorVersionId ? (
                <CheckCircle2
          ...
          <li className="rounded-xl border bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="font-medium">Tutor specification</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {tutorVersionId
                    ? "A compiled tutor version is active."
                    : "Waiting for a selected design to compile."}
                </p>
              </div>
              {tutorVersionId ? (
                <CheckCircle2
          ...
          <li className="rounded-xl border bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <p className="font-medium">Evaluation scenarios</p>
            ...
            <div
              ...
              aria-label={`${progress}% scenario progress`}
            >
              <div
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {failed
                ...
                : `${progress}% durable job progress`}
            </p>
          </li>
          <li className="rounded-xl border bg-card p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="font-medium">Evaluation run</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {scenarioCount === 6
                    ? "Ready to assess the tutor against all six scenarios."
                    : "Available after scenarios are ready."}
                </p>
              </div>
              {scenarioCount === 6 ? (
                <button
                ...
                >
                  {isEvaluating ? (
                    <span
                      aria-label="Running evaluation"
                      className="processing-ring processing-ring-small"
                    />
                  ) : (
                    "Run evaluation"
                  )}
        ...
        {tutorVersionId && scenarioCount !== 6 ? (
          <button
            type="button"
            onClick={() => void generate()}
            disabled={isGenerating || job?.status === "running"}
          ...
          >
            {isGenerating || job?.status === "running"
              ? "Generating scenarios…"
              : failed
                ? "Retry scenario generation"
                : "Generate six scenarios"}
          </button>
        ) : null}
        {error ? (
      ...
      <aside
        ...
        className="border-t bg-card p-5 lg:sticky lg:top-0 lg:max-h-[calc(100dvh-7.5rem)] lg:overflow-y-auto lg:border-l lg:border-t-0"
      >
        <h2 className="font-medium">Scenario generation</h2>
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Creates six course-grounded learner cases to check the tutor’s
          reasoning, hinting, scope boundaries, and answer-protection behavior
          before evaluation runs.
        </p>
        <div className="mt-5">
          <p className="text-sm font-medium">Generated previews</p>
          {isGenerating ? (
            <div className="mt-3 flex items-center gap-3 rounded-lg bg-muted p-3 text-sm font-medium">
              <span
                aria-label="Generating scenarios"
            ...
            <ul
              ...
              aria-label="Generated scenario previews"
            >
              {scenarios.map((scenario) => (
                <li key={scenario.id} className="rounded-lg bg-muted p-3">
                  <p className="text-sm font-medium leading-5">
                    {scenario.title}
                  </p>
                  <p className="mt-1 text-sm leading-5 text-muted-foreground">
                    {scenario.learnerMessages[0]}
                  </p>
                </li>
              ))}
            </ul>
