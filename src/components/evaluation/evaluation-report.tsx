...
export function EvaluationReport({ projectId }: { projectId: string }) {
  ...
  return <section className="max-w-4xl space-y-5" aria-labelledby="evaluation-report-heading">
    ...
    {report ? <button type="button" onClick={() => void continueToPreview()} disabled={advancing} className="inline-flex rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-60">{advancing ? "Opening Preview…" : "Continue to Preview"}</button> : null}
