...
const authorityOptions: ReadonlyArray<{
...
function sourceIsActive(source: SourceDocument) {
  return Object.values(source.processing).some(
...
export function SourceWorkspace({ projectId }: { projectId: string }) {
  ...
  const [permissions, setPermissions] =
    useState<SourcePermissions>(defaultPermissions);
  const [containsProtectedSolutions, setContainsProtectedSolutions] =
    useState(false);
  ...
  function chooseFiles(files: File[]) {
    ...
    if (accepted.length > remainingFiles) {
      localErrors.push(
        `Only ${Math.max(remainingFiles, 0)} source slots remain in this workspace.`,
      );
    }
    if (
      summary.bytes + accepted.reduce((total, file) => total + file.size, 0) >
    ...
    setNotice(
      accepted.length
        ? `${Math.min(accepted.length, Math.max(remainingFiles, 0))} file(s) ready to upload.`
        : "",
    );
  }
  ...
  async function analyzeAll() {
    ...
    try {
      ...
      if (!mutationIsCurrent()) return;
      showAnalysisJobResult(
        job,
        "ready course sources",
        refreshed,
        setNotice,
        setError,
      );
  ...
  async function remove(source: SourceDocument) {
    ...
    try {
      ...
      if (!mutationIsCurrent()) return;
      setError(
        cause instanceof Error ? cause.message : "Could not remove source.",
      );
    } finally {
      if (mutationIsCurrent()) setBusy(false);
    }
    ...
    <section className="space-y-8" aria-labelledby="sources-heading">
      ...
      >
        <BudgetMeter
          ...
          limit={DEFAULT_WORKSPACE_BUDGET.maxFiles}
          detail={`${remainingFiles} remaining`}
        />
        <BudgetMeter
          label="Storage"
          value={summary.bytes}
      ...
      >
        ...
        <label className="grid gap-2 text-sm font-medium">
          ...
          <input
            ...
            className="rounded-md border bg-background px-3 py-2 text-sm file:mr-3 file:rounded file:border-0 file:bg-primary/10 file:px-2 file:py-1 file:font-medium"
          />
        </label>
        {selectedFiles.length > 0 && (
          <p className="text-sm text-muted-foreground">
            Queued: {selectedFiles.map((file) => file.name).join(", ")} (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="grid gap-2 text-sm font-medium">
            Source authority
            <select
              aria-label="Source authority"
              value={authority}
              onChange={(event) =>
                setAuthority(event.target.value as SourceAuthority)
              }
              disabled={busy}
              className="rounded-md border bg-background px-3 py-2"
            >
        ...
        <fieldset className="grid gap-2 rounded-lg border p-4">
          ...
          <PermissionCheckbox
            ...
            checked={permissions.revealExcerptsToStudents}
            onChange={(value) =>
        ...
        <label className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
          <input
            type="checkbox"
            checked={containsProtectedSolutions}
            disabled={busy}
            onChange={(event) => {
              const checked = event.target.checked;
              setContainsProtectedSolutions(checked);
              if (checked)
          ...
          <span>
            <span className="font-medium">
              Contains protected answers or worked solutions
            </span>
            <span className="block text-amber-900">
              Student excerpts and runtime retrieval stay disabled for protected
              sources.
            </span>
          </span>
        </label>
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
          >
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4">
          <div>
            <h2 id="source-list-heading" className="font-semibold">
              Source processing
            </h2>
            <p className="text-sm text-muted-foreground">
              Processing details are metadata only; source excerpts are
    ...
    <tr className="border-b last:border-0">
      <td className="px-5 py-4 align-top">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {source.role.replaceAll("_", " ")} · {formatBytes(source.sizeBytes)}
        </p>
        {source.containsProtectedSolutions && (
          <p className="mt-2 inline-flex rounded bg-amber-100 px-2 py-1 text-xs font-medium text-amber-950">
            Protected solutions
          </p>
        )}
      </td>
