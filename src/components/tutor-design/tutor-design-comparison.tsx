...
export function TutorDesignComparison({ projectId, onCompile }: Props) {
  ...
  const receiveDesigns = useCallback((next: TutorDesign[], message: string) => {
    ...
    setOverrides(restoredId ? next.find((design) => design.id === restoredId)?.controls ?? null : null);
    setStatus(message);
  }, [projectId]);
  ...
  const choose = (design: TutorDesign) => {
    setSelectedId(design.id); setOverrides(design.controls); saveSelection(projectId, design.id);
  ...
  return <section className="space-y-6" aria-labelledby="tutor-design-heading">
    ...
    {designs.length > 0 && <>
      ...
      {designs.map((design) => {
        ...
        return <article key={design.id} className={`rounded-xl border bg-card p-5 shadow-sm ${isSelected ? "border-primary ring-1 ring-primary" : ""}`}>
          <div className="flex items-start justify-between gap-3"><h2 className="text-lg font-semibold">{design.title}</h2><span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-medium text-primary">{roles[design.candidateRole]}</span></div>
          ...
          <button type="button" aria-pressed={isSelected} onClick={() => choose(design)} className="mt-5 w-full rounded-md border px-4 py-2 text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">{isSelected ? "Selected" : `Choose ${design.title}`}</button>
        </article>;
      })}
      </div>
    </>}
    ...
    {selected && overrides && <form className="max-w-3xl space-y-5 rounded-xl border bg-card p-6 shadow-sm" onSubmit={(event) => { event.preventDefault(); void compile(); }}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Select label="Answer sharing" value={overrides.answerPolicy} options={[["never_reveal", "Never reveal final answers"], ["reveal_after_sufficient_attempts", "Reveal after sufficient attempts"], ["available_in_revision_mode", "Available in revision mode"]]} onChange={(answerPolicy) => setOverrides({ ...overrides, answerPolicy })} />
        <Select label="Tone" value={overrides.tone} options={[["encouraging", "Encouraging"], ["neutral", "Neutral"], ["formal", "Formal"]]} onChange={(tone) => setOverrides({ ...overrides, tone })} />
