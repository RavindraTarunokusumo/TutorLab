...
function exportedCourseSummary(courseModel: CourseModelVersionRecord, permittedIds: Set<string>) {
  const artifact = courseModel.artifact;
  return {
    schemaVersion: "0.1",
    courseIdentity: artifact.courseIdentity,
    structure: artifact.structure,
    learningObjectives: artifact.learningObjectives.map((item) => ({ ...item, evidence: item.evidence.filter((evidence) => permittedIds.has(evidence.documentId)) })).filter((item) => item.evidence.length > 0),
    concepts: artifact.concepts.map((item) => ({ ...item, evidence: item.evidence.filter((evidence) => permittedIds.has(evidence.documentId)) })).filter((item) => item.evidence.length > 0),
    terminology: artifact.terminology.map((item) => ({ ...item, evidence: item.evidence.filter((evidence) => permittedIds.has(evidence.documentId)) })).filter((item) => item.evidence.length > 0),
    methods: artifact.methods.map((item) => ({ ...item, evidence: item.evidence.filter((evidence) => permittedIds.has(evidence.documentId)) })).filter((item) => item.evidence.length > 0),
  };
}
...
export async function buildStandaloneTutorPackage(projectId: string): Promise<StandaloneTutorPackage> {
  ...
  const files = standaloneFiles(tutor, exportedCourseSummary(courseModel, permittedIds), { schemaVersion: "0.1", documents });
